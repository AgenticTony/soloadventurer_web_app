...
| 9e  | BOTH ⚠   | **Story 0.7 (NEW)** — phantom schema refs: 11 targets / 27 sites querying relations+RPCs that no migration creates. Fix `shared_meetups`, `message_reports`/`message_moderation`, `notify-new-message`'s `chats`+`chat_id`, web's 3 trip RPCs, `get_entries_near_location`, `travel_preferences`; 👤 decide `photos` | —          | ~1–2 wk | 🚨 **LAUNCH BLOCKER, 1/10.** The **class** behind 9a + 9d. CI ratchet landed (mobile); **web's fix is W.2 types**. Safety + cross-repo → **sign-off**. Report: `phantom-schema-refs-2026-07-16.md`.                                                            |
