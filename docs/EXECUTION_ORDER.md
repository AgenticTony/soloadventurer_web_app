...
> **Created:** 2026-07-05 · **Last updated:** 2026-08-13 · **Source of truth for _what_:** `docs/FOUNDATIONS.md` §9. This doc governs _order_ only. Mirror it into the mobile repo (like FOUNDATIONS) so both sides read the same sequence — **update both copies in the same change, or neither.**
...
- ✅ **Verification vendor swap — Onfido → Shufti Pro** — **merged 2026-08-12** (mobile PR #29 + follow-ups #33/#47). Schema is now vendor-neutral (`verification_records.provider`); signature verification is double-SHA256, proven against a live response, with tampered payloads rejected 401. **The UI was not connected** — both entry points called repository methods left as `UnimplementedError`, so no user could verify at all; fixed in PR #31, which also added the selfie step Shufti's face-match requires. `docs/sprints/SHUFTI_MIGRATION_PLAN.md`. **👤 Remaining: live (non-sandbox) credentials + one real-document test.**
- ✅ **PHASE_H (step 10c) — H.1–H.7 complete 2026-08-13**, plus a review of the phase with **all six findings fixed** (PR #47). See the 10c row below.
- ✅ **PHASE_W (step 9c) — W.2 / W.3 / W.4 complete 2026-08-13** (web PRs #28/#29/#30/#31). W.2's typed client surfaced **four phantom-_column_ bugs live in prod** — the class `check-schema-refs.py` cannot see, because it checks table and RPC names, not columns. **Remaining: Tailwind 3→4, `date-fns` v4.**
- ✅ **Release checklist + preflight gate** — **merged 2026-08-13** (mobile PR #44). `docs/RELEASE.md` + `scripts/preflight.py`, which diffs the **deployed** project against the repo: orphaned edge functions, migration-ledger drift, secrets, RLS coverage, anon-executable SECDEF. Written because every other gate reads the repository and none read production.
- 🚨 **Two prod-only defects found and fixed 2026-08-12**, both invisible to every repo-based check: **(a)** `verify-with-onfido` was deleted from the repo but stayed **ACTIVE in prod**, still carrying a stubbed signature check that writes `gender_verified` — now undeployed; **(b)** the Shufti DDL was applied via the Management API and **never written to the migration ledger**, so the next `db push` would have replayed the rebrand, hit a `RENAME COLUMN` on a column that no longer existed, and aborted mid-release — ledger now repaired. **This is the standing argument for running `scripts/preflight.py` before any release.**
...
| 9e  | BOTH ⚠   | **Story 0.7 (NEW)** — phantom schema refs: code querying relations, RPCs and edge functions that were never created. Remaining: `shared_meetups`, `notify-new-message`'s `chats`+`chat_id`, web's 3 trip RPCs, `get_entries_near_location`, `travel_preferences`, **`delete-user-account` (account deletion broken — GDPR/store)**; 👤 **deploy the remaining undeployed edge functions** — ~~incl. `verify-with-onfido`~~ **DO NOT deploy `verify-with-onfido`: it is deleted, and its webhook signature check is a stub that still writes `profiles.gender_verified`. It was found ACTIVE in prod on 2026-08-12 and removed; redeploying it reopens that hole.** | —          | ~1 wk | 🚨 **LAUNCH BLOCKER, 3/12.** The **class** behind 9a + 9d. ✅ 2026-07-17: **message reporting fixed + chat-UI wired** (repointed at `reports`; pgTAP-proven); **photos scaffold deleted**; ratchet extended to `.invoke()`. **Web's fix is W.2 types**. Safety + cross-repo → **sign-off**. `PHASE_0_BLOCKERS.md` Story 0.7.                                                                                                                                                            |
...
| 9c  | WEB       | **PHASE_W** — stack upgrade (Next 15→16, React 18.2→19.2, Tailwind 3→4); server-first Supabase refactor + generated types; retire `/feed`; reframe landing; test audit              | —          | 1–2 wk  | ✅ **W.2 / W.3 / W.4 DONE (2026-08-13).** W.2 generated types + typed clients — which surfaced **four phantom-column bugs** shipped to prod (reporting, chat read receipts, geographic matching, connection requests); W.3 `/feed` retired + landing copy reframed to the charter; W.4 schema-aware Supabase test double. **Remaining: Tailwind 3→4 and `date-fns` v4** (the W.1 majors).                                                                                                                                                                                                  |
...
| 10c | MOBILE ⚠ | **PHASE_H** — audit hardening: test the product core, error spine, women-only extraction, no-Supabase-in-UI, SECURITY DEFINER backfill, no-show dispute design, deps + Flutter 3.44 | 9a + 9b    | 2–3 wk  | ✅ **H.1–H.7 DONE (2026-08-13).** Plus a code review of the phase and **all six findings fixed** (mobile PR #47) — women-only premium-copy contradiction, a safety toggle that reverted after a successful write, a realtime subscription leak, a stale `currentUserIdProvider` after re-login, generic error labels, and offline reporting an active protection as "Disabled". Advisor backlog absorbed: `check_ins` RLS enabled, anon-executable SECDEF functions 17→0 (3 PostGIS builtins remain, not ours). **H.5's dispute design still gates public negative reputation (step 10).** |
