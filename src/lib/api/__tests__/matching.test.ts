import { findPotentialMatches, requestConnection, respondToConnection } from '../matching'
...
describe('findPotentialMatches', () => {
  ...
  describe('Tier 1 — semantic edge function', () => {
    it('returns CompositeMatch[] when edge function succeeds', async () => {
      ...
      expect(mockInvokeEdgeFunction).toHaveBeenCalledWith('find-potential-matches-semantic', {
      ...
      const results: SemanticMatchResult[] = [
        {
          ...
          sharedActivities: [],
          factors: { semantic: 0.5, dateOverlap: 0, activities: 0, destination: 0, age: 0 },
        },
        {
          userId: 'u2',
          displayName: 'B',
          ...
          sharedActivityCount: 0,
          sharedActivities: [],
          factors: { semantic: 0.9, dateOverlap: 0, activities: 0, destination: 0, age: 0 },
        },
      ]
  ...
  describe('Tier 3 — client-side with activity overlap', () => {
        ...
        if (table === 'user_activities') {
          ...
          return {
            select: jest.fn().mockReturnValue({
              ...
              in: jest
                .fn()
                .mockResolvedValue({ data: overrides?.otherActivities ?? [], error: null }),
            }),
          }
        }
        if (table === 'activities') {
          return {
            select: jest.fn().mockReturnValue({
              eq: jest
                .fn()
                .mockResolvedValue({ data: overrides?.activityNames ?? [], error: null }),
            }),
          }
        }
    ...
    it('includes shared activities in client-side match results', async () => {
      setupTier3Mocks({
        ...
        myActivities: [{ activity_id: 'act-1' }, { activity_id: 'act-2' }],
        otherActivities: [
...
describe('requestConnection', () => {
  ...
  describe('edge function primary path', () => {
    it('calls request-connection edge function with correct params', async () => {
      ...
      expect(mockInvokeEdgeFunction).toHaveBeenCalledWith('request-connection', {
        recipient_id: 'user-2',
        message: 'Hey there!',
      })
    })
  ...
  describe('validation error mapping', () => {
    ...
    it('maps "blocked" to user-friendly message', async () => {
      ...
      await expect(requestConnection('user-2')).rejects.toThrow(
        'Unable to send request — user is not available'
      )
    })
    ...
    it('maps "no active trip" to user-friendly message', async () => {
      ...
      await expect(requestConnection('user-2')).rejects.toThrow(
        'You need at least one active trip to connect'
      )
    })
    ...
    it('maps "self" connection to user-friendly message', async () => {
      ...
      await expect(requestConnection('current-user')).rejects.toThrow(
...
describe('respondToConnection', () => {
  ...
  describe('edge function primary path', () => {
    it('calls respond-connection edge function with accept=true', async () => {
      ...
      expect(mockInvokeEdgeFunction).toHaveBeenCalledWith('respond-connection', {
  ...
  describe('fallback to direct update', () => {
    ...
    it('falls back on timeout error', async () => {
      ...
      expect(mockUpdate).toHaveBeenCalledWith(expect.objectContaining({ status: 'declined' }))
    })
