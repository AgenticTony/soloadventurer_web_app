...
async function tryGeographicRpc(
  userId: string,
): Promise<CompositeMatch[] | null> {
  const supabase = createClient();
  ...
  try {
    ...
    return (data as OverlappingTripResult[]).map((row): CompositeMatch => ({
      id: row.id,
      userId: row.user_id,
      ...
      matchPercentage: row.overlap_days > 0 ? Math.round(Math.min(row.overlap_days / 7, 1) * 100) : 0,
      sharedActivityCount: 0,
      confidence: getMatchConfidence(
        row.overlap_days > 0 ? Math.round(Math.min(row.overlap_days / 7, 1) * 100) : 0,
      ),
      factors: { semantic: 0, dateOverlap: row.overlap_days > 0 ? Math.min(row.overlap_days / 7, 1) : 0, activities: 0, destination: 0, age: 0 },
    }));
  } catch (err) {
    console.warn(
  ...
  for (const trip of otherTrips ?? []) {
    ...
    for (const myTrip of myTrips) {
      const overlapStart = new Date(Math.max(
        new Date(myTrip.start_date).getTime(),
        new Date(trip.start_date).getTime(),
      ));
      const overlapEnd = new Date(Math.min(
        new Date(myTrip.end_date).getTime(),
    ...
    if (myTrips[0]?.latitude && myTrips[0]?.longitude && trip.latitude && trip.longitude) {
      distanceMeters = haversineDistance(
        myTrips[0].latitude, myTrips[0].longitude,
    ...
    if (activityFactor >= 0.3 && (distanceMeters !== null && distanceMeters < 50000 || maxOverlapDays > 0)) {
...
export async function requestConnection(recipientId: string, message?: string): Promise<Connection> {
...
function mapConnectionError(error: { message: string; status?: number; code?: string }): AppError {
  ...
  if (msg.includes('already connected') || msg.includes('already_exists') || msg.includes('duplicate')) {
...
async function requestConnectionFallback(recipientId: string, message?: string): Promise<Connection> {
  const { supabase, userId } = await getAuthContext();
