...
export async function getUserActivities(userId?: string): Promise<UserActivity[]> {
  ...
  const { data, error } = await supabase.from('user_activities').select('*').eq('user_id', targetId)
...
export async function setUserActivities(activityIds: string[]): Promise<void> {
  ...
  const { error: insertError } = await supabase.from('user_activities').insert(rows)
