  ...
  async getUserFeed(userId?: string): Promise<FeedItem[]> {
    try {
      ...
      const { data, error } = await supabase
        .from('feed_items')
        .select(
          'id, verb, object_id, object_type, created_at, actor:profiles!feed_items_actor_id_fkey(display_name, username, avatar_url)'
        )
      ...
      return (data ?? []).map((item: Record<string, unknown>) => ({
        ...
        title: formatFeedTitle(
          ...
          item.actor as Record<string, string> | null
        ),
