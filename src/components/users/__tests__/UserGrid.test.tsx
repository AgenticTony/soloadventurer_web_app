...
describe('UserGrid', () => {
  ...
  it('should show empty state when no users found', () => {
    render(<UserGrid {...defaultProps} users={[]} totalCount={0} />)
  ...
  it('should show error state', () => {
    render(<UserGrid {...defaultProps} users={[]} error="Failed to load users" />)
  ...
  it('should show initial loading skeletons when no users yet', () => {
    const { container } = render(<UserGrid {...defaultProps} users={[]} loading={true} />)
  ...
  it('should show end of results message when no more users', () => {
    render(<UserGrid {...defaultProps} hasMore={false} />)
  ...
  it('should display correct total count formatting', () => {
    // Test exact count
    render(<UserGrid {...defaultProps} users={mockUsers} totalCount={2} />)
    expect(screen.getByText('2 of 2 travelers')).toBeInTheDocument()

    // Test with more than current users (showing +)
    render(<UserGrid {...defaultProps} users={mockUsers} totalCount={50} />)
  ...
  it('should show error in results header when error occurs with existing results', () => {
    render(<UserGrid {...defaultProps} error="Connection failed" />)
