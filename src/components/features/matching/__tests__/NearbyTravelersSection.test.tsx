...
jest.mock('@/components/features/matching/MatchCard', () => ({
  MatchCard: ({
    ...
    match: { id: string; displayName: string; userId: string; sharedActivities: string[] }
  }) => (
...
const mockFindPotentialMatches = matchingApi.findPotentialMatches as jest.MockedFunction<
  typeof matchingApi.findPotentialMatches
>
const mockGetActivities = activitiesApi.getActivities as jest.MockedFunction<
  typeof activitiesApi.getActivities
>
...
function makeCompositeMatch(
  overrides: Partial<CompositeMatch> & {
    id: string
    userId: string
    displayName: string
...
describe('NearbyTravelersSection', () => {
  ...
  function findActivityChip(name: string) {
    return screen.getAllByRole('button').find(b => b.textContent?.trim() === name)
  }
  ...
  describe('activity filter chips', () => {
    ...
    it('shows "Clear" button when filters are active', async () => {
      ...
      const filtersButton = screen
        .getAllByRole('button')
        .find(b => b.textContent?.includes('Filters'))
    ...
    it('clears all filters when Clear button is clicked', async () => {
      ...
      const cookingButton = screen
        .getAllByRole('button')
        .find(b => b.textContent?.trim() === 'Cooking')
      fireEvent.click(cookingButton!)
    ...
    it('shows empty state with Clear Filters button when filter yields no results', async () => {
      ...
      const surfingButton = screen
        .getAllByRole('button')
        .find(b => b.textContent?.trim() === 'Surfing')
