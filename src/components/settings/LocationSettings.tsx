...
const SHARING_OPTIONS: SharingOption[] = [
  ...
  },
  {
    ...
    warning:
      'This setting makes your location visible to all users. Consider your privacy carefully.',
  },
]

export function LocationSettings({ className = '' }: LocationSettingsProps) {
  const { settings, updateLocationSharing, togglePreciseLocation, togglePrivacyStatus } =
    usePrivacy()

  const [showAdvanced, setShowAdvanced] = useState(false)

  const currentOption =
    SHARING_OPTIONS.find(opt => opt.value === settings.locationSharing) || SHARING_OPTIONS[0]
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      {/* Current Status */}
      <div className={`rounded-lg border-2 p-4 ${currentOption.color}`}>
        <div className="flex items-center gap-3">
          <currentOption.icon className="h-5 w-5" />
          <div>
            <div className="font-medium">Current Setting: {currentOption.label}</div>
            <div className="mt-1 text-sm opacity-90">{currentOption.description}</div>
          </div>
        </div>
        {currentOption.warning && (
          <div className="mt-3 flex items-start gap-2 rounded border bg-white/50 p-2">
            <AlertTriangle className="mt-0.5 h-4 w-4 flex-shrink-0 text-yellow-600" />
            <p className="text-xs text-yellow-800">{currentOption.warning}</p>
          </div>
        )}
      </div>
      ...
      {settings.locationSharing !== 'off' && (
        <div className="rounded-lg bg-gray-50 p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-start gap-3">
              <Eye className="mt-0.5 h-5 w-5 text-gray-600" />
              <div>
            ...
            <button
              ...
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${
                settings.preciseLocation ? 'bg-blue-600' : 'bg-gray-200'
              }`}
          ...
          {settings.preciseLocation && (
            <div className="mt-3 flex items-center gap-2 rounded border border-yellow-200 bg-yellow-50 p-2 text-sm text-yellow-800">
        ...
        <div className="space-y-4 rounded-lg bg-gray-50 p-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${
                settings.showPrivacyStatus ? 'bg-blue-600' : 'bg-gray-200'
              }`}
          ...
          <div className="rounded border border-gray-200 bg-white p-3">
            <h4 className="mb-2 font-medium text-gray-900">Current Privacy Level</h4>
            ...
            <div className="mt-1 text-xs text-gray-500">
              ...
              {settings.hideFromUsers.length > 0 &&
                `Hidden from ${settings.hideFromUsers.length} users • `}
