...
# --- S3: private media bucket with CORS ---
...
resource "aws_s3_bucket_ownership_controls" "media" {
  bucket = aws_s3_bucket.media.id
  rule { object_ownership = "BucketOwnerEnforced" } # ACLs off
}
...
resource "aws_cognito_user_pool" "users" {
  ...
  schema { name = "name"         attribute_data_type = "String" required = true  mutable = true }
  schema { name = "phone_number" attribute_data_type = "String" required = false mutable = true }
}
...
# --- AppSync: GraphQL API (Cognito auth) + stub schema ---
resource "aws_appsync_graphql_api" "api" {
  ...
  log_config { field_log_level = "ERROR" }
}

resource "aws_appsync_graphql_schema" "schema" {
  api_id = aws_appsync_graphql_api.api.id
  definition = <<EOF
type User { id: ID!, email: String!, name: String!, avatar: String }
type Query { _health: String }
schema { query: Query }
EOF
}

# --- SSM: output important values for the app ---
